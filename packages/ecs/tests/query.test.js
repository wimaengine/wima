...
describe("Testing `Query`", () => {
  test('query for single component, single entity', () => {
    ...
    strictEqual(
      typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentA.constructor)),
      typeid(A)
    )
  })
  ...
  test('query for multiple components, single entity', () => {
    ...
    strictEqual(
      typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentB.constructor)),
      typeid(B)
    )
    strictEqual(
      typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentC.constructor)),
      typeid(C)
    )
  })
  ...
  test('query for single component, multiple entities', () => {
    ...
    query.each(([component]) => {
      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(component.constructor)),
        typeid(A)
      )
    })
  })
  ...
  test('query for combination of entities', () => {
    ...
    query.eachCombination(([componentA1, componentB1, componentC1],[componentA2, componentB2, componentC2]) => {
      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentA1.constructor)),
        typeid(A)
      )
      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentB1.constructor)),
        typeid(B)
      )
      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentC1.constructor)),
        typeid(C)
      )

      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentA2.constructor)),
        typeid(A)
      )
      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentB2.constructor)),
        typeid(B)
      )
      strictEqual(
        typeid(/**@type {import("@wimaengine/type").Constructor}*/(componentC2.constructor)),
        typeid(C)
      )
      count += 1
    })
